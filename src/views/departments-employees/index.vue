...
import { delDepartmentEmp, fetchDepartmentEmpList } from '@/service/api/department-employees';
...
function handleEdit(row: UserData) {
  ...
  // 实现编辑功能
  router.push({
    path: `departments-employees/detail`,
    query: {
      ...row
    }
  });
...
function handleAdd() {
  console.log('新增用户');
  router.push('/departments-employees/detail');
  // 实现新增功能
}
...
function search() {
  ...
  fetchDepartmentEmpList(params).then((res: any) => {
    ...
    <NCard title="Search Conditions" class="mb-4">
      <NForm :model="searchForm" label-placement="left" label-width="80px" class="search-form">
        <NGrid :cols="4" :x-gap="16">
          ...
          <NFormItemGi label="Dept_Number">
            <NInput v-model:value="searchForm.Dept_Number" clearable />
          </NFormItemGi>
